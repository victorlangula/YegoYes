<ion-view title="{{ $page->name }}">
    <ion-content padding="true" class="{{ $app->content_classes }}" ng-controller="CouponCtrl" ng-init="sl = '{{ $sl }}'; code = '{{ $coupon->code }}'; redeemed = ''">
<?php
if(! $found)
{
?>
...
<?php } ?>
					...
					<button class="button button-large button-block button-balanced" id="redeem" ng-show="redeemed == '0'" ng-btn-ok="{{ trans('widget::global.redeem') }}" ng-btn-cancel="{{ trans('widget::global.cancel') }}" ng-really-title="{{ trans('widget::global.redeem_coupon') }}" ng-really-message="{{ trans('widget::global.confirm_redeem') }}" ng-really-click="redeemCoupon('{{ $coupon->code }}')">
					{{ trans('widget::global.redeem') }}
					</button>

					<span class="button button-large button-block icon-left ion-android-done button-assertive" ng-show="redeemed == '1'" style="cursor:default;">{{ trans('widget::global.redeemed') }}</span>
