...
<?php } ?>
			<div class="item item-body">
                <p style="margin-top:0"><span style="float:left;width:18px;text-align:center"> <i class="ion-calendar"></i></span> {{correctTimestring('<?php echo $event->event_start ?>') | date:'mediumDate'}}<?php if ($event->event_end != '') { ?> - {{correctTimestring('<?php echo $event->event_end ?>') | date:'mediumDate'}}<?php } ?></p>
