...
class AccountController extends \BaseController {
    ...
    public function getLoginFacebook()
    {
		...
		elseif ($state == '' || ($state !== $_SESSION['oauth2state'])) 
		{
			...
			try {
				...
				$oauth_user = $provider->getUserDetails($token);

				// Check if user already exists or not
				$user = \User::where('facebook', $oauth_user->uid)->first();
				...
				if ($user == NULL)
				{
					...
					$user->parent_id = NULL;
					$user->username = $oauth_user->uid;
					$user->facebook = $oauth_user->uid;
					$user->plan_id = 1;
    ...
    public function getLoginTwitter()
    {
		...
		if (! empty($oauth_token) && ! empty($oauth_verifier)) {
			...
			// Check if user already exists or not
			$user = \User::where('twitter', $oauth_user->uid)->first();
    ...
    public function getOrderSubscription()
    {
		...
		if($sl != '')
		{
			...
			$new_date = (\Auth::user()->expires != NULL) ? \Auth::user()->expires : \Carbon::now();
			$new_date_month = $new_date->addMonth()->format("Y-m-d");
			$new_date_year = $new_date->addYear()->format("Y-m-d");
			...
			{
				$diff_days_end_of_plan = $new_date->diffInDays(\Carbon::now());
