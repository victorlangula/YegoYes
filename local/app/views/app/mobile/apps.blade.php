...
{
?>
                    <div class="pull-right col-xs-12 col-sm-auto"><a href="#/app" class="btn btn-primary btn-labeled" style="width: 100%;"><span class="btn-label icon fa fa-plus"></span> {{ trans('global.new_app') }}</a></div>
<?php
}
?>
	...
	<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4 campaign campaign{{ $app->campaign_id }}" id="app{{ $i }}">
		<div class="panel panel-default panel-dark panel-body-colorful widget-profile widget-profile-centered widget-profile-default">
			...
			<div class="widget-profile-counters">
				...
				<div class="col-xs-6">
					<a href="#/app/{{ $sl }}" class="btn btn-primary btn-xs" data-toggle="tooltip" title="{{ trans('global.edit_app') }}"><i class="fa fa-pencil fa-1x"></i></a>
...
<script>
$.getJSON("{{ url('/api/v1/stats/visits?sl=' . $sl) }}", function(data) {
	$('#app{{ $i }} .stat-visits').html('<a href="#/stats/{{ $sl }}">' + data.visits + '</a>');
});
</script>
<?php
	$i++;
...
else
{
	...
	{
		echo '<div class="callout pull-right arrow-right-up" style="margin-right:40px">' . Lang::get('global.create_first_app') . '</div>';
	}
}
?>
