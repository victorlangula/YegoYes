...
<ul class="breadcrumb breadcrumb-page">
	...
	<?php
		...
		?>
	<li class="active">{{ trans('global.data_and_statistics') }}</li>
	<?php
		} elseif($app === false && isset($campaign->name)) {
		?>
	<li><a href="#/stats">{{ trans('global.data_and_statistics') }}</a></li>
	...
	<?php
		...
		?>
	<li><a href="#/stats">{{ trans('global.data_and_statistics') }}</a></li>
	<li><a href="#/stats/{{ $sl_campaign }}">{{ $campaign->name }}</a></li>
...
<?php
	$title = (isset($campaign->name)) ? $campaign->name : trans('global.data_and_statistics');
				...
				<div class="pull-right col-xs-12 col-sm-auto">
					<div class="btn-group" style="width:100%;">
						...
						<ul class="dropdown-menu dropdown-menu-right" role="menu">
							<li><a href="#/stats/{{ $date_start }}/{{ $date_end }}/" tabindex="-1">{{ trans('global.remove_filter') }}</a></li>
							<li class="divider"></li>
							<?php
								$campaign_name_old = '';
								foreach($apps as $app_select)
								{
									if($campaign_name_old != $app_select->campaign_name)
									{
										...
										echo '<li class="nav-header nav-link' . $class . '"><a href="#/stats/' . $sl_campaign . '">' . $app_select->campaign_name . '</a></li>';
									}
								...
								?>
							<li class="{{ $class }}"><a href="#/stats/{{ $date_start }}/{{ $date_end }}/{{ $sl_app }}" tabindex="-1">{{ $app_select->name }}</a></li>
							<?php
								}
								?>
						</ul>
	...
	columnDefs: [
		{
			render: function (data, type, row) {
				return '<span data-moment="fromNowDateTime">' + data + '</div>';
			},
...
<script>
	...
	$('#stats-range').on('apply.daterangepicker', function(ev, picker) {
	    ...
	    document.location = '#/stats/' + start + '/' + end + '/{{ $sl }}';
	});
