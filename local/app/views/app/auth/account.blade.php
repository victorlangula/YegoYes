...
?>
				<div class="plan-col col-sm-3">
					...
					<ul class="plan-features">
						<li class="plan-align-left">{{ trans('admin.apps') }}: {{ $apps }}</li>
						<li class="plan-align-left">{{ $domain_icon . ' ' . trans('global.custom_domain') }}</li>
						<li class="plan-align-left">{{ $download_icon . ' ' . trans('global.download_app') }}</li>
						<li class="plan-align-left">{{ trans('admin.widgets') }}: {{ $plan_widgets_count }} <a href="javascript:void(0);" onclick="$('.widget-details').toggle()">[?]</a></li>
<?php
foreach ($widgets as $widget_name => $widget)
{
	if ($widget['active'])
	{
		...
		echo '<li class="widget-details plan-align-left" style="display:none">' . $included . ' ' . $widget_name . '</li>';
	}
}
?>
...
if (empty($orders))
{
	echo '<p class="lead">' . trans('admin.no_invoices') . '</p>';
}
else
{
?>
