...
CmsApp.config(function ($routeProvider) {
        ...
        when('/stats', {
            templateUrl: 'app/stats',
			active: 'stats'
        }).
        when('/stats/:start/:end', {
            templateUrl: function(params) { return 'app/stats?start=' + params.start + '&end=' + params.end; },
			active: 'stats'
        }).
        when('/stats/:sl', {
            templateUrl: function(params) { return 'app/stats?sl=' + params.sl; },
			active: 'stats'
        }).
        when('/stats/:start/:end/:sl', {
            templateUrl: function(params) { return 'app/stats?start=' + params.start + '&end=' + params.end + '&sl=' + params.sl; },
			active: 'stats'
        }).
        when('/campaigns', {
...
CmsApp.controller('MainCtrl', function($route, $routeParams, $scope, $location, $templateCache, $rootScope){
	$scope.$on('$routeChangeStart', function(scope, newRoute){
		// Loading animation
		$('#content-wrapper').html('<div class="spinner" ng-controller="MainCtrl" id="spinner"> <div class="rect1"></div> <div class="rect2"></div> <div class="rect3"></div> <div class="rect4"></div> <div class="rect5"></div> </div>');
